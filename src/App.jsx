...
// ─── ZOOM API — routed via Supabase Edge Function to avoid CORS ──────────────
// Zoom's OAuth endpoint blocks direct browser requests (CORS policy).
// All Zoom calls go through: Supabase → Edge Function → Zoom API → back to browser.

async function getZoomToken(accountId, clientId, clientSecret) {
  // Route through Supabase Edge Function to avoid CORS
  const res = await fetch(`${SUPABASE_URL}/functions/v1/zoom-proxy`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      "Authorization": `Bearer ${SUPABASE_ANON}`,
    },
    body: JSON.stringify({ action: "token", accountId, clientId, clientSecret }),
  });
...
async function createZoomMeeting({ accountId, clientId, clientSecret, agentEmail, topic, startTime, duration = 60 }) {
  ...
  if (!res.ok) throw new Error(`Proxy error ${res.status}`);
  const data = await res.json();
  if (!data.join_url) throw new Error(data.error || "Failed to create meeting");
